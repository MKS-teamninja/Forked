...
routes.post('/review/', bodyParser.json(), function (req, res) {
    buckets.updateReview(req.body.user_id, req.body.rest_id, req.body.user_rating, req.body.review, req.body.price).then(function (data) {
        console.log('restaurant review edited', data);
        res.status(202).json(data);
    }).catch(function (err) {
        console.log('restaurants post err', err);
        res.status(500).send('Server Error');
